...
DATA_TYPES = [
    ...
    'VARCHAR(100)',
    'VARCHAR(255)',
    'VARCHAR(255) CHARACTER SET Unicode NOT CaseSpecific',
    'VARCHAR(500)',
    'VARCHAR(5000)',
    'LONG VARCHAR',
    'FLOAT',
...
@st.cache_data(show_spinner=False)
def get_help(_session, database, table, counter):
    ...
    for column, dtype_dict in dtypes.items():
        ...
        if not dtype_dict['dtype']:
            ...
            if max_len<=11:
                ...
                dtype_dict['dtype'] = 'VARCHAR(255)'
            elif max_len <= 255:
                dtype_dict['dtype'] ='VARCHAR(255) CHARACTER SET Unicode NOT CaseSpecific'
            elif max_len<=500:
    ...
    if st.session_state.main_page == 'welcome':
        ...
        def run_create(new_table_name):
            ...
            if error_text:
                ...
                log(f'created table {st.session_state.database}.{new_table_name}')
                st.session_state.main_page = 'mapping'
        ...
        if 'table' in st.session_state:
            columns_dict = get_help(session, st.session_state.database, st.session_state.table, st.session_state.counter)
        else:
